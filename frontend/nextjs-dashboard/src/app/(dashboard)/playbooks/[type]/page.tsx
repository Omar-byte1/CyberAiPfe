...
export default function PlaybookExecutionPage() {
  ...
  const toggleTask = (taskId: string) => {
    ...
    if (!completedTasks.includes(taskId)) {
      setToast({
        message: `Step ${taskId} validated. AI is monitoring for progress...`,
        type: "success",
      });
    }
  };
    ...
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        ...
        <p className="font-bold text-slate-400 uppercase tracking-widest text-xs">
          Deploying Incident Core...
        </p>
      </div>
    );
  }

  const currentTaskData = phases
    .flatMap((p) => p.tasks || [])
    .find((t) => t && t.id === activeTask);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-2">
                {phase.tasks?.map((task) => (
                  ...
                  >
                    ...
                    <ArrowRight
                      className={`w-4 h-4 transition-all ${activeTask === task.id ? "text-blue-500 translate-x-0 opacity-100" : "-translate-x-2 opacity-0"}`}
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="p-8 bg-emerald-500/10 border-2 border-dashed border-emerald-500/30 rounded-[2.5rem] text-center space-y-4 animate-in zoom-in duration-500">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-slate-500">
                  Incident successfully contained. Generating Post-Incident
                  Report...
                </p>
              </div>
              <button
                onClick={() => {
                  setToast({
                    message: "Generating summary for management...",
                    type: "info",
                  });
                }}
        ...
        <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-xs text-slate-500 font-medium italic leading-relaxed">
                      Attention: Assurez-vous d'avoir l'approbation du
                      management avant d'effectuer des modifications réseau
                      critiques sur les serveurs de production.
                    </p>
                  </div>
                </div>
          ...
          <div className="p-6 bg-indigo-600 rounded-[2rem] text-white shadow-xl shadow-indigo-600/20 relative overflow-hidden group">
            ...
            <p className="text-xl font-black italic tracking-tighter mb-4 leading-none">
              AI CORE IS AUTO-CORRELATING LOGS FOR THIS INCIDENT
            </p>
